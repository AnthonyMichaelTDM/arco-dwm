...
CONFIG_DIRECTORY="~/Downloads/" #for end user: change this to be the directory you stored my configs in
readonly CONFIG_DIRECTORY

CURSOR_SOURCE="~/.icons/BloodMoon-Cursor/"
CURSOR_TARGET="/usr/share/.icons/"
readonly CURSOR_SOURCE
readonly CURSOR_TARGET

GTK_SOURCE="~/themes/Bloodmoon/"
...
BACKGROUND_IMAGE_SOURCE="/usr/share/backgrounds/arcolinux/"
...
DWM_MAKE_DIR="~/.config/arco-dwm/"
ST_MAKE_DIR="~/.config/arco-st/"
SLSTATUS_MAKE_DIR="~/.config/arco-slstatus/"
readonly DWM_MAKE_DIR
readonly ST_MAKE_DIR
readonly SLSTATUS_MAKE_DIR

SCRIPT_SOURCE="${CONFIG_DIRECTORY}install-script.sh"
...
case $brightnessYN in
	y|Y ) #modify sxhkdrc and .zshrc-personal, also copy the brightness changing script, and make it so that said script can run without needing a sudo password
...
sudo sed -i 's/Current=/Current=BloodMoon-sugar-candy # /' $SDDM_CONF
sudo sed -i 's/CursorTheme=/CursorTheme=BloodMoon-Cursor # /' $SDDM_CONF
echo "done"
...
sed -i 's/gtk-theme-name=/gtk-theme-name=BloodMoon # /' GTK3_SETTINGS
sed -i 's/gtk-cursor-theme-name=/gtk-cursor-theme-name=BloodMoon-Cursor # /' GTK3_SETTINGS
echo "done"
